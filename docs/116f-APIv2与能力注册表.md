...
| **116f-D** ✅ 已完成（2026-08-15）Flutter typed DTO 样板 | ① `core/dto/` MarketProduct/DashboardStats typed model（解析容错与旧页面取值口径逐项一致）② `core/repository/` `ProductRepository` 接口 + `HttpRepository`（经 `ApiClient.v2`，可注入 fake）③ `ApiClient.v2` 只读命名空间（与 v1 共用 token/自动刷新/信封解析链，仅前缀不同）④ 迁移 `product_detail_page.dart` 到 repository+DTO，行为不变（选商品页，对应 D3 market product 样板；`data_dashboard_page.dart` 未迁移、DashboardStats DTO 已备）⑤ DTO/仓储/页面迁移单测 | 新增 `app/lib/core/dto/market_product.dart`、`app/lib/core/dto/dashboard_stats.dart`、`app/lib/core/repository/product_repository.dart`、`app/lib/core/repository/http_repository.dart`、`app/test/dto_market_product_test.dart`、`app/test/dto_dashboard_stats_test.dart`、`app/test/dto_repository_test.dart`、`app/test/dto_product_detail_page_test.dart`；编辑 `api_client.dart`（kV2Prefix + v2 命名空间 + `_getPath` 抽取，v1 行为不变）、`product_detail_page.dart` | ✅ 全部满足（2026-08-15）：flutter test **34/34**（原 13 + 新 21）；B14-B16 不回归；页面 412×732 浏览器截图与迁移前构建**逐像素一致**（0 差异像素/301,584，Edge headless + 临时库副本） |
| **116f-E** ✅ 已完成（2026-08-16）管理台目录生成化 + 资源组去重 | ① 新增 `scripts/gen-admin-api-catalog.mjs` 从注册表生成 `apiCatalog.js`：唯一事实源=注册表，覆盖全部 242 条 v1（key=apiId、method/path/auth/roles 同源、auth 映射 required→true/optional→false），26 条 v1 调试预设以 overlay 装饰保留（含 /admin/* 4 条，均挂载于 /api/v1、本就在注册表内），预设 method+规范化 path 必须恰好命中 1 条注册表条目否则生成失败（fail-fast）② `resourceGroups.js` 与后端 `resource.config.js` 的 `aiDetectRecord` 去重：agri 唯一 primaryGroup，ai 组 `resources:['aiQaRecord']` + `tags:['aiDetectRecord']` 表达（D2）③ B19/B20 契约更新（重复=0、覆盖完整、可确定性重建、双镜像一致） | 新增 `backend/scripts/gen-admin-api-catalog.mjs`；生成 `backend/admin/src/apiCatalog.js`；编辑 `backend/admin/src/resourceGroups.js`、`backend/src/modules/platform/resource.config.js`、`backend/admin/test/apiCatalog-resourceGroups.test.js` | ✅ 全部满足（2026-08-16）：admin test **39/39**（原 28 + 新 11）、admin build 绿；`gen-admin-api-catalog.mjs --check` 漂移门禁通过；B19 重建一致 + 重复=0 + 注册表逐字段一致 + 覆盖完整 + 26 预设保留；B20 去重 + admin/backend 双镜像一致；v1/v2/capability 数量不变（242/242、5/5、247）；village.db 指纹不变 |
...
- 2026-08-11：工作单建立。只读盘点完成（源码证据见 §1/§2）；未实施任何代码改动。GitHub 网络不可达，`679a87d8` 尚未推送（详见交接报告）。
...
- 2026-08-15（116f-B 实施并验收 + 集中审查整改）：
  ...
  - **未实施**：116f-C（market product v2 样板）、Flutter feature_catalog 生成、OpenAPI 工具链、`aiDetectRecord` 实际去重与 B20 契约修改（仅在注册表 migrationNotes 记录 D2 迁移方案）；本批已随提交入库，未 push。
- 2026-08-15（116f-C 实施并验收）：
  ...
  - **未实施**：116f-D（Flutter typed DTO 样板）及后续批次、Flutter/Admin 调用方迁移、feature_catalog 生成、OpenAPI、`aiDetectRecord` 去重与 B20；本批已随提交入库，未 push。
- 2026-08-15（116f-D 实施并验收）：
  - **范围（严格按 §9 116f-D）**：① `core/dto/` MarketProduct/DashboardStats typed model；② `core/repository/` ProductRepository 接口 + HttpRepository；③ `ApiClient.v2` 只读命名空间；④ 迁移 `product_detail_page.dart`（选商品页，对应 D3 样板）到 repository+DTO，行为不变；⑤ DTO 解析单测。**未实施**：116f-E/F、dashboard 页迁移、全量 135 调用点迁移、feature_catalog 生成、OpenAPI、`aiDetectRecord` 去重与 B20。
  - **外部路径（mount-relative）**：样板页 `HttpRepository.fetchDetail` → `ApiClient.v2.get('/market/products/:id')` → 外部 `GET /api/v2/market/products/:id`（mount-relative `/market/products/:id`，与 v1 `GET /market/product/:id` 同 controller 同数据同 optional 语义）。v1 前缀/路径/行为零改动，不全局改前缀（§5.7）。
  - **复用证据**：`HttpRepository` 零 HTTP/认证/错误处理逻辑（只调 `ApiClient.v2.get` 后 `MarketProduct.fromJson`）；`ApiClient.v2` 复用 `_getPath/_sendAuthenticated/_parse` 同一链路（token/自动刷新/统一信封/测试注入共用）；页面 getter 回退链与迁移前逐项一致（title/unit/price 缺失保持 null 走 preview 回退、category 缺省「农产品」、images `'$e'` 过滤空串、seller 投影、regionCode/traceCode 空白归一为 null）；DashboardStats 容错镜像 `data_dashboard_page.dart` 的 `_int/_double/_text/_map/_list` 助手语义。
  - **新增测试 21 项（`app/test/dto_*.dart`）**：market_product 7（完整 payload 与 seller 投影、缺失/null 回退、显式空串保留、images 字符串化与空串过滤、空白归一、price/seller 类型错误与旧页面同口径）；dashboard_stats 4（完整聚合 payload、statReport 外层缺失回退 dataJson、缺失/异常块容错、字符串数值收敛与 'null' 文本回退）；repository 5（接口可注入 fake、`ApiClient.v2` 命中 `/api/v2/market/products/:id` 且携带 Bearer 头实证、data null/非对象返回 null、业务码非 200 抛 ApiException）；product_detail_page 5（正向字段全部来自仓储响应且与 preview 兜底不同——证明命中仓储而非 fallback、stock=0 禁用、错误屏+重试恢复、productId null、preview 回退链）。
  - **验收**：Flutter **34/34**（原 13 + 新 21），B14/B15/B16 不回归；Backend 182/182；C2b 19/19；C2c 34/34；Admin 28/28 + build；`scripts/verify-all.ps1` **15/15**（node --check 106 文件；116f inventory/registry drift 两步均过）；口径不变：v1 242/242、v2 5/5、capabilities 247；正式 village.db 指纹不变（SHA256 `FAEECC...E96` / 1155072B / 2026-08-07T08:34:59.2995944Z）；`backend/prisma/.test-*` 残留 0；全仓库额外 DB 文件 0；Prisma schema/migrations 零改动；未改 116d/116e 状态；`HARD_COVERAGE_GATE_ALL_ENVIRONMENTS` 未提前翻转。
  - **视觉验收**：迁移前构建（08-11 `build/web`，页面直连 v1）与迁移后构建（116f-D，页面走 v2+DTO）在 Edge headless 412×732 视口、同一临时库副本后端、同一商品（id=64）下截图（CDP 登录态注入 + 深链 `#/market/product/64`），**逐像素一致（0 差异像素 / 301,584）**；临时库副本位于 `$env:TEMP`、用完即删，正式库零接触。
  - **未实施**：116f-E（管理台目录生成化 + 资源组去重）及后续批次。**116f-E 未开始**：须待 116f-D 提交核对通过并收到用户另行指令后方可启动，本批未授权自动进入。
- 2026-08-16（116f-E 实施并验收）：
  - **范围（严格按 §9 116f-E）**：① 新增 `backend/scripts/gen-admin-api-catalog.mjs`（唯一事实源=注册表；`--write`/`--check`；输出确定、预设 fail-fast 校验）；② `backend/admin/src/apiCatalog.js` 改为注册表生成产物：**11 组 242 条 v1**（key=apiId、method/path/auth/roles 同源、auth 映射 required→true / optional→false），26 条既有 v1 调试预设以 `DEBUG_PRESETS` overlay 装饰保留（§1.2 旧口径「24 条」实际文件为 26 条，全部保留；`/admin/*` 端点挂载于 `/api/v1` 下（platform.routes.js）、本就在注册表内，故无第二套人工清单）；③ `backend/admin/src/resourceGroups.js` 与 `backend/src/modules/platform/resource.config.js` 的 `aiDetectRecord` 去重；④ B19/B20 契约更新。
  - **资源组去重口径**：去重前 9 组 36 条登记（35 唯一，`aiDetectRecord` 在 agri 与 ai 跨组重复）；去重后 9 组 **35 条登记且 35 唯一（重复=0）**。ai 组改为 `resources:['aiQaRecord']` + `tags:['aiDetectRecord']`（D2 的 tag/secondaryGroup 表达）；`RESOURCE_CONFIGS`（字段/模型配置）与业务 controller 零改动；`/admin/resource/*` 全部 `requireAuth+requireRole('ADMIN')`（platform.routes.js L75-81），资源分组仅为菜单/目录用途、不含 per-resource 角色矩阵——去重不改变任何角色/权限/菜单语义；`listResourceConfigs` 的 find 首匹配语义不变（aiDetectRecord 仍归 agri）；admin 与 backend 两份资源组镜像逐组一致（B20 断言）。
  - **生成化与 drift 口径**：apiCatalog 242 条 = 注册表 v1 242 条（B19 双向覆盖断言）；method+规范化 path 重复=0；`buildCatalog()` 与提交产物 `deepEqual`（可确定性重建）；26 条预设名称逐条在场 + `dashboard`/`market-products`/`farm-record-create`/`ai-qa-thread`/`switch-list` 代表项强断言（示例 query 路径、请求体、auth、roles 均保留）。`node scripts/gen-admin-api-catalog.mjs --check` 与既有 inventory/gen-capabilities 两个 drift 门禁全部通过；verify-all.ps1 未改（「drift 检查入 verify-all」属 116f-F ③）。
  - **测试与验收**：Admin **39/39**（原 28 + 新 11：28 − 12（HEAD B19 7 项 + B20 5 项）+ 23（当前 B19 13 项 + B20 10 项）= 39）+ admin build 绿；Backend 182/182；C2b 19/19；C2c 34/34；Flutter 34/34；`scripts/verify-all.ps1` **15/15**；v1 242/242、v2 5/5、capabilities 247 不变；正式 village.db 指纹不变（SHA256 `FAEECC...E96` / 1155072B / 2026-08-07T08:34:59.2995944Z）；`backend/prisma/.test-*` 残留 0；全仓库额外 DB 文件 0；Prisma schema/migrations 零改动；注册表 capabilities.js 与 migrationNotes 未改（116f-E 文件边界不含注册表本体）。
  - **未实施**：116f-F 及后续批次；116f-E 的提交状态与 hash 以 git history 为准。
- 2026-08-16（116f-F 实施并验收）：
  - **范围（严格按 §9 116f-F）**：① `assistant.service.js` 的 ROUTE_CATALOG/ROUTE_FEATURES 改为注册表派生（手工表删除，B9 `sanitizeAssistantOutput` 白名单行为不变）；② Flutter `feature_catalog.dart` 由注册表生成（构建期脚本，离线安全，D5）；③ 三个生成器的 drift 检查入 verify-all；④ `backend/test/registry-derive.test.js`（C5）。**未实施**：后续批次（OpenAPI 工具链、v1 删除等，§10 不属 116f）。
  - **事实源分层（生成产物与人工维护源不得混称）**：人工可编辑事实源 = `gen-capabilities.mjs` 的 `FEATURE_CATALOG` overlay（sections/routes/routeFeatures/features 数据本体）；`backend/src/contracts/capabilities.js` 是其 `--write` 生成产物（下游规范注册表快照，新增顶层 `featureCatalog` 区块）；assistant 运行时消费 `assistant-catalog.generated.js`、Flutter 消费 `feature_catalog.dart`、Admin 消费 `apiCatalog.js`——三端均只消费自己的生成产物，不存在第二套手写清单。**capabilities 数组不变 → 247 = 242(v1) + 5(v2) 不变**。**口径修正**：§1.3 盘点旧口径「feature_catalog 71 项 / ROUTE_CATALOG 32 条」与实际文件不符——实测 feature_catalog **70 项**、ROUTE_CATALOG **34 条**（与前端 `_routePaths` 34 键一致）；以实际文件为准全部保留（同 116f-E「24→26」先例），docs 以 70/34 为正式口径。
  - **生成器与产物**：`gen-capabilities.mjs`（overlay 扩充 + `validateFeatureCatalog` fail-fast：routes key 唯一、feature name 唯一、routeKey 已登记、route 与页面 path 一致、icon 为合法 Dart 标识符、section 已登记、别名页内无重复）；新增 `scripts/gen-assistant-routes.mjs` → `src/modules/ai/services/assistant-catalog.generated.js`（ROUTE_CATALOG 34 页 + ROUTE_FEATURES 16 页，`assistant.service.js` 从中 import，ALLOWED_ROUTE_KEYS 仍由 ROUTE_CATALOG 派生）；新增 `scripts/gen-feature-catalog.mjs` → `app/lib/core/feature_catalog.dart`（FeatureItem 类 + kFeatureSections 8 + kFeatureCatalog 70，icon 存 Material 常量名渲染为 `Icons.xxx`，Dart 字符串转义确定）。三个生成器均 `--write`/`--check`，`--check` 只读、发现 drift 退出非零；不依赖数据库/网络/时间/随机数/工作树顺序。
  - **verify-all**：新增「116f generated catalog drift」步骤（gen-admin-api-catalog + gen-assistant-routes + gen-feature-catalog 三个 `--check` 顺序执行、失败传播非零）；drift 子门禁总数 2 → **5**（inventory-routes、gen-capabilities、gen-admin-api-catalog、gen-assistant-routes、gen-feature-catalog）；verify-all 15 → **16 步**。
  - **测试与验收**：新增 `backend/test/registry-derive.test.js` **33 项**（assistant 派生、admin 派生、Flutter 派生、drift 门禁、HEAD 基线冻结五组）：派生结果与注册表逐字段一致、双向完整覆盖、key 唯一、method+规范化 path 唯一、auth/roles/version 关键字段精确、代表性 v1/v2/公开/认证/ADMIN/参数化路径、重复=0、stale/空产物门禁失败、`--check` 不改写文件（内容+mtime 前后一致）、正向结果与空目录明显不同、HEAD 旧行为基线冻结（规范化 SHA256 + 代表项 + 正/负对照）。Backend 182 → **215/215**（182 + 33）；C2b 19/19、C2c 34/34、Flutter 34/34（B14/B15/B16 不回归）、Admin 39/39+build；`scripts/verify-all.ps1` **16/16** 全绿；v1 242/242、v2 5/5、capabilities 247 不变；正式 village.db 指纹不变（SHA256 `FAEECC...E96` / 1155072B / 2026-08-07T08:34:59.2995944Z）；`backend/prisma/.test-*` 残留 0；全仓库额外 DB 文件 0；Prisma schema/migrations 零改动；`HARD_COVERAGE_GATE_ALL_ENVIRONMENTS` 未翻转（D6 的硬门禁升级不在 116f-F 文件边界）。
