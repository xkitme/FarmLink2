...
在 P16 Flutter 骨架基础上，补齐首批可演示业务页面，让移动端不再只停留在框架和占位：
...
Flutter Web 构建提示 `CupertinoIcons` 字体未找到，但项目代码未引用 `CupertinoIcons`，且构建成功，不影响当前演示页面。
...
- 把政策、集市、农机页面从静态演示数据切换为后端 API 数据。
- 接入离线缓存兜底。
