...
把分段 17 的静态 Flutter 业务页面推进到可演示状态：页面优先读取本机 Node.js 后端，失败时使用本地缓存或静态演示兜底；关键操作可以形成“点得动、能返回、可讲清”的闭环。
...
- Ollama 未启动时，后端仍可返回 `local-rule-rag` 兜底答案，适合断网比赛演示。

### 6. 后端请求兼容

- `backend/src/app.js`
- 在全局链路中加入 `optionalAuth`，让携带 token 的非强制登录接口也能拿到用户上下文。
- 已有 `/ai`、`/agri` 路由仍保留 `requireAuth`，比赛演示时需要先登录。
...
- 比赛断网时，如果 Ollama 没启动，AI 问答会自动进入 SQLite RAG + 本地规则兜底，不会整块废掉。
- 集市下单、农机预约目前使用固定演示收货信息和预约日期，后续可改成表单。
