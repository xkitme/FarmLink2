...
export function systemPrompt(scene) {
  const common = '你是数字乡村助农平台的本地离线助手。回答要通俗、稳妥、可执行；不知道时说明依据不足，不编造政策编号、药剂剂量或法律结论。'
  const map = {
...
export function fallbackAnswer({ scene, question, references }) {
  ...
  if (references.length) {
    const intro = {
      POLICY: '根据本地惠农政策库，先为你整理这些要点：',
      AGRI: '根据本地农技知识库，建议这样处理：',
      DISASTER: '根据本地应急预案，建议按这些步骤处理：',
      LIFE: '根据本地乡村服务信息，整理如下：',
      EDU: '可以按这个思路来理解：',
      GENERAL: '根据本地知识库，整理如下：',
    }[normalized] || '根据本地知识库，整理如下：'
    return `${intro}\n${sourceLines(references)}\n\n建议：先按上述要点核对自身情况；涉及补贴、理赔、法律或用药的事项，请再向村委、农技员或主管部门确认。`
  }

  const generic = {
    POLICY: '暂未在本地政策库检索到直接条款。建议换一个更具体的问法，例如补贴名称、作物类型、所在地区；也可以向村委或乡镇农业农村部门确认。',
    AGRI: '暂未检索到直接农技条目。建议补充作物、症状、发生时间、田间照片和近期施肥用药情况，便于进一步判断。',
    DISASTER: '暂未检索到对应应急预案。建议优先保障人身安全，拍照留证，联系村委或乡镇农业服务站，并保留保险理赔材料。',
    LIFE: '暂未检索到直接服务信息。建议补充所在村镇、服务类型和联系方式，便于匹配本地资源。',
  }[normalized] || '暂未检索到直接资料。可以换个更具体的说法，我会继续从本地库里帮你找。'
  return generic
}
