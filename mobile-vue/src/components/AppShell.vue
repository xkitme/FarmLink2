    ...
    <nav class="bottom-nav" aria-label="主导航">
      <button v-for="item in navItems" :key="item.name" type="button" class="bottom-nav__item"
        ...
        <span class="bottom-nav__icon">
          <van-icon :name="item.icon" :size="item.name === 'publish' ? 24 : 21" />
          <span v-if="item.name === 'messages' && unread > 0" class="bottom-nav__badge">{{ unread > 99 ? '99+' : unread }}</span>
        </span>
...
export default {
  ...
  computed: {
    ...
    unread() { return (this.$store.state.notification && this.$store.state.notification.unread) || 0 }
  },
  created() {
...
.bottom-nav__icon { position: relative; display: grid; place-items: center; width: 40px; height: 28px; border-radius: 999px; }
.bottom-nav__badge { position: absolute; top: -3px; right: 2px; min-width: 16px; height: 16px; padding: 0 4px; color: #fff; font-size: 10px; font-weight: 700; line-height: 16px; text-align: center; background: #ba1a1a; border: 1.5px solid #fff; border-radius: 999px; box-sizing: border-box; }
