  ...
  <SubPage title="AI 助手" fallback="/home">
    ...
    <van-pull-refresh v-model="refreshing" @refresh="onRefresh">
      <van-list v-model="loading" :finished="finished" finished-text="没有更多记录了" @load="loadMore">
        <button v-for="t in items" :key="t.threadId || t.id" type="button" class="thread" @click="open(t)">
...
.thread { display: flex; flex-direction: column; width: 100%; margin-bottom: 10px; padding: 14px; text-align: left; background: #fff; border: 1px solid #e5dfce; border-radius: 14px; }
